...
namespace System.Net.Sockets
{
    ...
    internal sealed class SocketAsyncContext
    {
        ...
        private abstract class ReadOperation : AsyncOperation, IThreadPoolWorkItem 
        {
            ...
            public override void TryCancel() => AssociatedContext._receiveQueue.TryCancelQueued(this);
        }
        ...
        private abstract class WriteOperation : AsyncOperation, IThreadPoolWorkItem
        {
            ...
            public override void TryCancel() => AssociatedContext._sendQueue.TryCancelQueued(this);
        }
            ...
            where TOperation : AsyncOperation
        {
            ...
            public bool IsReady(SocketAsyncContext context, out int observedSequenceNumber)
            {
                using (Lock())
                {
                    ...
                    bool isReady = !_stopped && _tail == null;
                ...
                while (true)
                {
                    ...
                    using (Lock())
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // The queue is empty and an event has occurred since we previously checked.
                                // Retry the operation before we enqueue it.
            ...
            public void TryCancelQueued(AsyncOperation op)
            {
                bool removed = TryRemoveQueued(op);
                if (removed)
                {
                    op.AbortAndDispatch();
                }
            ...
            public bool TryRemoveQueued(AsyncOperation op)
            {
                using (Lock())
                {
                    if (_tail != null)
                    {
                        ...
                        else
                        {
                            ...
                            while (current.Next != op && current != _tail)
                            {
            ...
            public void Trace(SocketAsyncContext context, string message, [CallerMemberName] string memberName = null)
            {
                ...
                OutputTrace($"{IdOf(context)}-{queueType}.{memberName}: {message}, {(_stopped ? "stopped" : "ready")}-{_sequenceNumber}, {((_tail == null) ? "empty" : "not empty")}");
            }
        }
            ...
            where TOperation : AsyncOperation
        {
            ...
            using (var e = new ManualResetEventSlim(false, 0))
            {
                ...
                if (!e.Wait(timeout) &&                // timed out
                    queue.TryRemoveQueued(operation))  // not completed
                {
